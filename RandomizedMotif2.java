/*programa que recibe como parametro de entrada los enteros k y t, seguido por una colección de cadenas de ADN y da como resultado una colección de cadenas BestMotifs resultantes de correr RandomizedMotif ( Dna , k , t) . */
import java.io.*;
import java.util.Scanner;
class RandomizedMotif2
{
    public static void main(String [] args)
    {
        ...
        while (contador2 < 5000)
        {
            ...
            while (contador == 0)
            {
                ...
                for (int nc=0; nc < numero_cadenas; nc++)
                {
                    aux_arreglo=probables(cadenas_adn[nc], kmer, m_count, numero_cadenas);
                    for (ii=0; ii < kmer; ii++)
                        motif_nuevo[nc][ii]=aux_arreglo[ii];
                }
                m_count    =count(motif_nuevo, kmer, numero_cadenas);
            ...
            contador2++;
            m_count    =count(motif, kmer, numero_cadenas);
            score_nuevo=score(m_count, kmer, numero_cadenas);
            //System.out.println("score_anterior"+score_anterior2+"score_nuevo:"+score_nuevo);
            if (score_nuevo < score_anterior2)
            {
                score_anterior2=score_nuevo;
                motif_nuevo    =motif;
        ...
        for (int tc=0; tc < longitud_cadenas; tc++)
        {
            ...
            for (ikmer=0; ikmer < kmer; ikmer++)    //recorre los arreglos
            {
                if (indice[ikmer] >= 0 && valor_letra != -1)//si el índice de la cadena es menor al número k-mer y la letra tiene un valor diferente de -1
                {
                    ...
                    probabilidad[ikmer]                   =probabilidad[ikmer] * (probabilidad_letra / contador_cadenas + 4); //multiplica la probabilidad
                }
